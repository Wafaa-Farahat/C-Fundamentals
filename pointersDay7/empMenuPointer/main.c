...
int handleMenuSelection(int selected, struct Employee* employees, int employeeCount);
struct Employee* allocateEmployees(int* employeeCount);
void addEmployee(struct Employee* employees, int employeeCount);
void displayEmployee(const struct Employee* employees, int employeeCount);
...
int main() {
    ...
    // Allocate dynamic memory for employees
    struct Employee* employees = allocateEmployees(&employeeCount);
    ...
    while (1) {
        ...
        if (key == 224 || key == 0) {  // Handle arrow keys
            ...
            if (!handleMenuSelection(selected, employees, employeeCount)) {
...
// Function to allocate dynamic memory for employees
struct Employee* allocateEmployees(int* employeeCount) {
...
void addEmployee(struct Employee* employees, int employeeCount) {
    int index;
    printf("Enter an index (1 to %d) to fill employee data, or 0 to stop: ", employeeCount);
    scanf("%d", &index);
...
void displayEmployee(const struct Employee* employees, int employeeCount) {
    int index;
    printf("Enter an index (1 to %d) to display employee data, or 0 to go back: ", employeeCount);
    scanf("%d", &index);

    if (index == 0) return;

    if (index < 1 || index > employeeCount || employees[index - 1].code == 0) {
